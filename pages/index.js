// pages/index.js — UX destekli son sürüm
import { useEffect, useState, useRef } from 'react';
import axios from 'axios';
import { Flame, Lightning, TrendingDown, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      coins.forEach(coin => {
        ...
        if (track30 && track30.length) change30s = ((current - track30[0]) / track30[0]) * 100;
        if (track60 && track60.length) change60s = ((current - track60[0]) / track60[0]) * 100;
        ...
        const v = volatilityTrack.current[symbol] || [];
        if (v.length >= 5) {
          const max = Math.max(...v), min = Math.min(...v), avg = v.reduce((a, b) => a + b, 0) / v.length;
          const spike = ((max - min) / avg) * 100;
          if (spike >= 1.5) isVolatile = true;
        }

        let isMomentumUp = change10s > 0 && change30s > change10s && change60s > change30s;
        let isMomentumDown = change10s < 0 && change30s < change10s && change60s < change30s;

        if (current && base10) {
          result.push({ symbol, currentPrice: current.toFixed(4), change10s: change10s?.toFixed(2), change30s: change30s?.toFixed(2), change60s: change60s?.toFixed(2), volumeChange: volumeChange?.toFixed(2), isHotVolume, isVolatile, isMomentumUp, isMomentumDown });
      ...
      setUpCount(up);
      setDownCount(down);
      const sorted = result.sort((a, b) => b.isHotVolume - a.isHotVolume || Math.abs(b.change10s) - Math.abs(a.change10s));
  ...
  const toggleFollow = (symbol) => {
    setFollowedCoins(prev => prev.includes(symbol) ? prev.filter(s => s !== symbol) : [...prev, symbol]);
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-4">
      ...
      <div className="max-w-7xl mx-auto">
        <h1 className="text-center text-2xl font-bold text-lime-400 mb-6">📈 Binance Futures Realtime Monitor</h1>
              ...
              {displayCoins.map((coin) => (
                <tr key={coin.symbol} className="border-b border-zinc-800 hover:bg-zinc-800 transition">
                  <td className="px-3 py-2 font-mono">
                    {coin.symbol} {coin.isVolatile && <Flame className="inline text-orange-500" size={16} />} {coin.isMomentumUp && <Lightning className="inline text-green-400" size={16} />} {coin.isMomentumDown && <TrendingDown className="inline text-red-500" size={16} />}
                  </td>
                  ...
                  <td className="px-3">
                    <button onClick={() => toggleFollow(coin.symbol)} className="text-xs px-2 py-1 rounded bg-zinc-700 hover:bg-zinc-600">
          ...
          <div className="mt-8 bg-zinc-900 p-4 rounded-xl">
            ...
            <ul className="space-y-1 text-sm">
              {followedCoins.map(symbol => (
                <li key={symbol}>
                  <a href={`https://www.binance.com/en/futures/${symbol}`} target="_blank" rel="noreferrer" className="text-white hover:underline">
                    {symbol}
                  </a>
                </li>
              ))}
